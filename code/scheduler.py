...
def addPhoneme(p, t):
    ...
    global f
    if p != prevPhoneme:
        strings[4] += f"{str.format('{0:.3f}', t)},phoneme,{p}\n"
    prevPhoneme = p
...
def pickNewPose(t):
    ...
    prevPose = pose
    pose = newPose
    strings[3] += f"{str.format('{0:.3f}', t)},pose,{str(pose)}\n"
    prevPhoneme = "na"
...
mouthList = [
    ...
    ["zh", "t"],
    ["oov", "m"],
]  # For unknown phonemes, the stick figure will just have a closed mouth ("mmm")
...
print("Remapping Phonemes for Simple...")
if SIMP:
    for x in mouths:
        if mouths[x] == "m":
            mouths[x] = "a"
...
for i in range(WORD_COUNT):
    ...
    if (
        "\n" in nextDigest
        and data["words"][i - 1]["case"] != "not-found-in-audio"
        and (
            prevPhoneme == "a"
            or prevPhoneme == "f"
            or prevPhoneme == "u"
    ...
    if "<" in nextDigest:
        ...
        strings[1] += f"{str.format('{0:.3f}', timeStart)},emotion,{str(emotion)}\n"
        prevPhoneme = "na"

    if "\n\n" in nextDigest:
        pararaph += 1
        image += 1  # The line of the script advances 2 lines whenever we hit a /n/n.
        strings[0] += f"{str.format('{0:.3f}', timeStart)},paragraph,{str(pararaph)}\n"
        prevPhoneme = "na"

    if "\n" in nextDigest:
        image += 1
        strings[2] += f"{str.format('{0:.3f}', timeStart)},image,{str(image)}\n"
        prevPhoneme = "na"
        if not pickedPose:
...
with open(f"{INPUT_FILE}_schedule.csv", "w+") as f:

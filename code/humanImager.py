...
import pygame
import random
from utils import getFilenameOfLine, getTopic, capitalize
...
START_DRAW_GAP = (
    -1
)  # How many milliseconds after a frame change that the drawing is disabled, to prevent accidentally drawing on Image 2 as soon as it flips over. (I disabled this by making it negative.)
LINE_ON = 0
INK_COLOR = None
...
def refreshCanvas():
    ...
    if LINE_ON < len(lines):
        font_surface = canvasFont.render(
            capitalize(getTopic(lines[LINE_ON])), True, (0, 0, 0)
        )
...
def switchLines(first):
    ...
    if (
        not first
    ):  # We don't want to save any images on the very first initialization of the program.
        if not os.path.isdir(B_FOLDER):
            os.mkdir(B_FOLDER)
        pygame.image.save(
            canvassy, B_FOLDER + "/" + getFilenameOfLine(lines[LINE_ON]) + ".png"
        )
    while LINE_ON < len(lines) and os.path.exists(
        B_FOLDER + "/" + getFilenameOfLine(lines[LINE_ON]) + ".png"
...
while running:
    ...
    if (
        LINE_ON < len(lines)
        and pygame.time.get_ticks() >= lastLineSwitchTime + LINE_TIME_LIMIT
    ):
        switchLines(False)
    timeFracLeft = (
        lastLineSwitchTime + LINE_TIME_LIMIT - pygame.time.get_ticks()
    ) / LINE_TIME_LIMIT
